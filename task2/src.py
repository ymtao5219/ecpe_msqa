...
if __name__=='__main__':
    ...
    openai.api_key = "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"
    correct = 0
    num_data = 0
    indexs = []
    targets = []
    ...
    # document = ''
    # for i in files['2']['text']:    
    #     document += ', '.join(i) + ' '
    # document = document.strip()
    # print(document)
    # text_prompt = 'Given this text:' + document + 'Extract emotion-cause pairs and return the answer as a list of tuples clause numbers of emotion and cause. '
    # completion = openai.Completion.create(model="text-davinci-003", prompt=text_prompt, temperature=0.7, max_tokens=256)
    # response = completion.choices[0].text
    # print(response)
    # print(target)
